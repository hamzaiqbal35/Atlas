...
export const StaggeredMenu: React.FC<StaggeredMenuProps> = ({
  ...
  className,
  logoUrl = '/atlas_logo_transparent.png',
  menuButtonColor = '#fff',
  ...
  const playClose = useCallback(() => {
    ...
    closeTweenRef.current = gsap.to(all, {
      ...
      onComplete: () => {
        ...
        const scrollEl = panel.querySelector('.sm-panel-inner');
        if (scrollEl) {
  ...
  const playCloseAndAnimate = useCallback(() => {
      playClose();
      animateIcon(false);
      animateColor(false);
      animateText(false);
  ...
  const toggleMenu = useCallback(() => {
    ...
    animateColor(target);
    animateText(target);
  }, [playOpen, playCloseAndAnimate, animateIcon, animateColor, animateText, onMenuOpen, onMenuClose]);
  ...
  }, [playCloseAndAnimate, onMenuClose]);
            ...
            {items && items.length ? (
              items.map((it, idx) => {
                ...
                <li className="sm-panel-itemWrap" key={it.label + idx}>
                  <Link 
                    className={`sm-panel-item ${isParentActive ? 'active' : ''}`} 
                    href={it.children && it.children.length > 0 ? '#' : it.link} 
                  ...
                  {it.children && it.children.length > 0 && (
                    <div className={`sm-panel-sublist ${expandedItems[it.label] ? 'expanded' : ''}`}>
                      <ul role="list">
